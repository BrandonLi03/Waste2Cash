...
class address_popup : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_address_popup, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        val inputNewAddress = view.findViewById<EditText>(R.id.et_address)
        val change_btn = view.findViewById<Button>(R.id.btn_change)
        val cancel_btn = view.findViewById<Button>(R.id.btn_cancel)

        change_btn.setOnClickListener {
            val newAddress = inputNewAddress.text.toString()
            if (newAddress.isNotEmpty()) {
                databaseHelper.updateAddress(userId, newAddress)
                dismiss()
            }
        }
